...
protected:
    ...
    void adjustCompression()
    {
        ...
        {
            if (StringFind(headers[i][0], "sec-websocket-extensions") == 0 && StringFind(headers[i][1], "permessage-deflate") >= 0)
            {
                return;
            }
        }
    ...
    int readPacketFromNet(uchar &data[], const bool waitForHeaders = false)
    {
        ...
        } while (((socket.isReadable() && read > 0) || (waitForHeaders && read != -1 && !_LastError)) && !IsStopped());
...
public:
    ...
    virtual bool handshake(const string url, const string host, const string origin, const string custom = NULL)
    {
        ...
        WsTools::parseHeaders(response, headers); // re-fill headers from response
